...
switch ($mode) {
    ...
    default:
        $isChecked = array_key_exists($permission->code, $permissionsData)
            && $permissionsData[$permission->code] == 1;
        break;
}
?>
...
">
    <div class="item-content">
        ...
        <label class="item-name">
            ...
            <?php if (isset($permission->comment)): ?>
                <i class="icon-info-circle" title="<?= e(__($permission->comment)) ?>" data-bs-toggle="tooltip"></i>
            <?php endif ?>
        </label>
    </div>
    <?php if (isset($permission->children) && $permission->children): ?>
        <ul class="child-items">
            <?php foreach ($permission->children as $childPermission): ?>
                <?= $this->makePartial('permission_item', ['permission' => $childPermission]) ?>
            <?php endforeach ?>
        </ul>
    <?php endif ?>
</li>
